...
class Bridge(object):
    ...
    def __init__(self, device='udpin:192.168.2.1:14550', baudrate=115200, source_system=1, source_component=1):
        ...
        """
        self.conn = mavutil.mavlink_connection(device, baud=baudrate, source_system=source_system, source_component=source_component)
    ...
    def set_cmd_vel(self, pwm_linear_x=1500, pwm_linear_y=1500, pwm_linear_z = 1500 ,pwm_angular_z=1500):
        ...
        rc_channel_values[4] = int(pwm_linear_x)
        rc_channel_values[5] = int(pwm_linear_y)
        rc_channel_values[2] = int(pwm_linear_z)   # UP-DOWN
        self.conn.mav.rc_channels_override_send(
            self.conn.target_system,  # target_system
            self.conn.target_component,  # target_component
            *rc_channel_values)  # RC channel list, in microseconds.
   ...
   '''
    ...
    def get_data_force_torque_sensor(self):
        if self.data.keys().__contains__('NAMED_VALUE_INT') is True and self.data.keys().__contains__('DEBUG_VECT') is True:
            ...
            f_y_raw = self.get_data()['NAMED_VALUE_INT']['value']
            f_z_raw = int(self.get_data()['DEBUG_VECT']['name'])
            m_x_raw = int(self.get_data()['DEBUG_VECT']['x'])
            m_y_raw = int(self.get_data()['DEBUG_VECT']['y'])
            m_z_raw = int(self.get_data()['DEBUG_VECT']['z'])
            # TODO: SISTEMARE L'OFFSET
            f_mean = (f_x_raw + f_y_raw + f_z_raw)/3
            m_mean = (m_x_raw + m_y_raw + m_z_raw)/3
            f_x_raw = f_x_raw - f_mean
            f_y_raw = f_y_raw - f_mean
            f_z_raw = f_z_raw - f_mean
            m_x_raw = m_x_raw - m_mean
            m_y_raw = m_y_raw - m_mean
            m_z_raw = m_z_raw - m_mean

            [f_x, f_y, f_z, m_x, m_y, m_z] = np.matmul(self.calib_mat, np.matrix([f_x_raw, f_y_raw, f_z_raw, m_x_raw, m_y_raw, m_z_raw]).T)
            #print([f_x, f_y, f_z, m_x, m_y, m_z])
...
if __name__ == '__main__':
    ...
    while True:
        bridge.update()
        bridge.print_data()  # print all data
