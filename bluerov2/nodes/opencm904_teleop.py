...
class KeyBoardVehicleTeleop:
    def __init__(self):
        ...
        if self._msg_type == 'twist':
            self._output_pub = rospy.Publisher('/opencm904/control', Twist, queue_size=1)
        else:
            self._output_pub = rospy.Publisher('/opencm904/control', Accel, queue_size=1)
    ...
    def _parse_keyboard(self):
        ...
        if key_press != '':
            ...
            # If no button is pressed not reset to 0 to simulate ho joystick work 
            self.l = self.l
            self.a = self.a
